...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (telefonoInput) {
        // Función auxiliar para mostrar feedback de validación
        function mostrarFeedbackTelefono(input, esValido, mensaje) {
            if (!input) return;
        ...
        telefonoInput.addEventListener('blur', function() {
            ...
            if (valor) {
                if (valor.length < 6) {
                    mostrarFeedbackTelefono(this, false, 'El teléfono debe tener al menos 6 caracteres');
                } else if (valor.length > 20) {
                    mostrarFeedbackTelefono(this, false, 'El teléfono no puede exceder 20 caracteres');
                } else if (!pattern.test(valor)) {
                    mostrarFeedbackTelefono(this, false, 'Solo se permiten números y símbolos (+, -, paréntesis)');
                } else {
                    mostrarFeedbackTelefono(this, true, '');
                }
            } else {
    ...
    function mostrarFeedbackValidacion(input, esValido, mensaje) {
        ...
        // Buscar o crear elemento de feedback
        // Para selects, el feedback debe estar después del select, no dentro del parent
        let feedback = null;
        if (input.tagName === 'SELECT') {
            // Para selects, buscar el feedback después del select o en el contenedor
            ...
            // Para inputs, buscar en el parent
            feedback = input.parentElement.querySelector('.invalid-feedback');
            if (!feedback && !esValido) {
                feedback = document.createElement('div');
                feedback.className = 'invalid-feedback';
