...
/**
 * Obtiene la imagen grupal de una categoría
 * Busca archivos con "grupal" en el nombre en la raíz de la carpeta de categoría
 * 
 * @param string $categoria Nombre de la categoría (ej: "Camisas", "Blusas", "Shorts", "Pantalones")
 * @return string Ruta de la imagen grupal o imagen por defecto
 */
function obtenerImagenGrupalCategoria($categoria) {
    $categoria_lower = strtolower($categoria);
    $ruta_categoria = "imagenes/productos/{$categoria_lower}";
    
    // Si la carpeta existe, buscar imágenes grupales
    if (is_dir($ruta_categoria)) {
        $archivos = scandir($ruta_categoria);
        $imagenes_grupales = array();
        
        foreach ($archivos as $archivo) {
            // Buscar archivos que contengan "grupal" en el nombre
            if (preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $archivo) && 
                preg_match('/grupal/i', $archivo)) {
                $imagenes_grupales[] = $ruta_categoria . '/' . $archivo;
            }
        }
        
        // Si hay imágenes grupales, devolver la primera
        if (!empty($imagenes_grupales)) {
            // Priorizar imágenes que no sean específicas de género
            $imagenes_prioridad = array();
            $imagenes_secundarias = array();
            
            foreach ($imagenes_grupales as $img) {
                $nombre = basename($img);
                // Si contiene "hombre" o "mujer", es secundaria
                if (preg_match('/(hombre|mujer)/i', $nombre)) {
                    $imagenes_secundarias[] = $img;
                } else {
                    $imagenes_prioridad[] = $img;
                }
            }
            
            // Devolver primera de prioridad, o primera secundaria si no hay prioridad
            if (!empty($imagenes_prioridad)) {
                return $imagenes_prioridad[0];
            } elseif (!empty($imagenes_secundarias)) {
                return $imagenes_secundarias[0];
            }
        }
    }
    
    // Si no se encuentra imagen grupal, buscar en subcarpetas
    // Para Shorts, puede estar en la raíz con nombre diferente
    if ($categoria_lower === 'shorts') {
        // Buscar cualquier imagen en la raíz de shorts que sea grupal
        $imagenes_alternativas = array(
            'imagenes/productos/shorts/pantalon_mujer_grupal.png',
            'imagenes/productos/shorts/pantalon_mujer_grupal_exterior.png',
            'imagenes/productos/shorts/pantalon_mujer_grupal_noche.png'
        );
        
        foreach ($imagenes_alternativas as $img) {
            if (verificarArchivoExiste($img)) {
                return $img;
            }
        }
    }
